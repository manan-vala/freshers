...
function LoginPage() {
  ...
  const onSubmit = (data: LoginInput) => {
    loginMutation.mutate(data, {
      onSuccess: (user) => {
        // Redirection is handled by the beforeLoad guard of the destination route
        // or we can explicitly navigate. Let's rely on the router.
        window.location.href = user.student?.onboardingStatus === 'SUBMITTED' 
          ? '/onboarding-complete' 
          : '/onboarding';
      },
      onError: (error: any) => {
        // We'll set a generic form error or use the backend message
        form.setError('root', {
          type: 'manual',
